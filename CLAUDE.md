...
- **`lib/rails-mermaid_erd.rb`** — declares the `mermaid_erd` Rake task. It calls `Builder.model_data`, then evaluates `lib/templates/index.html.erb` with `version`, `app_name`, `logo`, `result` (the schema dump), and `viewer_defaults` in scope, and writes the rendered HTML to the path from `Configuration#result_path`. ERB binding is the entire integration contract between Ruby and the front-end.

- **`lib/rails-mermaid_erd/builder.rb`** — the only nontrivial Ruby. Calls `Rails.application.eager_load!`, walks `ActiveRecord::Base.descendants`, and emits `{Models: [...], Relations: [...]}`. Three things to know before touching it:
  1. **One relation line per stored link, not per model pair.** Every association is filed under the link it describes (`link_key`): a foreign key column (`belongs_to` on the model holding it, `has_many`/`has_one` on the model it references), a join table (both sides of HABTM), or — for `through:` associations, which store nothing of their own — the pair of models. Only after every model is read does `relation_for` turn each group into one line, so glyphs and grouping never depend on which model sorts first. A foreign key line always puts the referenced model on the left: `||` or `|o` (if any `belongs_to` on that column is optional), then `o{`, or `o|` when only `has_one` points back. Two associations on different columns between the same models are deliberately two lines. Resolve a new association kind through `link_key` rather than searching `Relations` for a line to append to.
  ...
  3. **Single table inheritance is drawn per table.** A subclass sharing its base class's table (`Complaint < Comment`) is not an entity of its own: its associations are walked with the base class's (`hierarchy_reflections`, de-duplicated on name + target so inherited ones aren't drawn twice), and any association targeting a subclass is resolved to the base class name. Resolve new target names through the same `sti_base_names` map, or the diagram gets orphan subclass nodes.

  4. **A column is a key when the database or an association says so.** Primary keys come from `primary_key` (an array on Rails 7.1+ for a composite key, so every column of it is `PK`). A column is `FK` if a foreign key constraint covers it, or if a `belongs_to` names it (polymorphic included, its `*_type` column is not a key), or if a `has_many` / `has_one` on another model names it on this table. Many applications add no constraints, so the constraint alone leaves them with no `FK` columns and Keys Only shows just `id`. A `belongs_to` marks its own column even when its target is in `ignore_tables`.
...
- **`lib/rails-mermaid_erd/configuration.rb`** — reads `config/mermaid_erd.yml` from the host app via `Rails.root`. Keys: `result_path` (defaults to `mermaid_erd/index.html`), `ignore_tables`, and `viewer_defaults` (`models` and `columns`, the view the viewer opens on and returns to on Reset). Keep new keys backwards-compatible (the merge happens on top of a hardcoded default hash), and validate values eagerly with an `ArgumentError` that names the key.

- **`lib/templates/index.html.erb`** — ~870 lines of Vue 3, Mermaid.js, and Tailwind, all in one file. The three front-end bundles are vendored under `lib/templates/vendor/` and inlined at render time (see `RailsMermaidErd.read_gem_asset` in `lib/rails-mermaid_erd.rb`) so the generated HTML has no CDN dependency at runtime — see `lib/templates/vendor/README.md` for the refresh procedure, `CHECKSUMS.txt`, and `LICENSES.md`. Schema is injected via `window.SCHEMA_DATA = <%= result.to_json %>`, and the configured starting view via `window.VIEWER_DEFAULTS` (a saved state in the hash wins over it; `reset` and a hash that leaves a field out fall back to it). UI state (selected models, toggles) is serialized to the URL hash as base64 JSON, so links are shareable. Any new field you add in `Builder` is visible here automatically via that JSON dump — coordinate naming. i18n strings live in the `window.i18n` block; both `en` and `ja` must be kept in sync. Mermaid 10+ made `mermaid.render` asynchronous, so `reRender` must stay `async`/`await`; the call sites are fire-and-forget on purpose.
