...
| `viewer_defaults` | Вид, с которым открывается просмотрщик, когда в URL нет сохранённого состояния, и к которому возвращает **Сбросить**; ссылка со своим состоянием всегда имеет приоритет. `models`: массив имён моделей, выбранных изначально (имя, которого нет в ERD, пропускается с предупреждением при генерации файла). `columns`: `all`, `keys` (только первичные и внешние ключи) или `none` — те же варианты, что и «Столбцы» в боковой панели. Удобно, чтобы в большом приложении сразу открывать те модели, с которых обычно начинают. | `models: []` (ничего не выбрано), `columns: all` |
